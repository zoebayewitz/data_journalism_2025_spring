...
**Task** Load the five CSV files. Do they have the same column structure? **Answer: Yes, they have the same column structure.**
...
**Task** Run the following code to join Maryland population data and the 2020 voter registration data together using the common column county as the key. How many rows are there now? How many *should* there be? **Answer: There are 24 rows; there should be 24, one for each county.**
...
**Task** Create a new dataframe with population and voter data joined together. What does the output message below say about the join? **Answer: the dataframe is joining by county.**
...
**Task** Using mutate, let's find the county with the highest 18+ Black population as a percentage of all population 18 and over and also the percentage of Democrats in that county. Put your answers below. **Answer: the county with the highest 18+ population and the highest percentage of democrats is Prince George's county.**
...
**Task** Now let's find the county with the highest 18+ Asian population as a percentage of all population 18 and over and also the percentage of Democrats in that county. How do these results compare to the previous task? Put your answers below. **Answer: these results show a much weaker correlation between race and likeliness of being a democrat. The correlation in the previous task was nearly one-to-one, while in this dataset there is much more fluctuation.**
...
**Task** Run the following code to load the Zip Code Tabulation Area data for Maryland. What's the datatype of the ZCTA5N? column? **Answer: ZCTA5N is a <dbl> and shows just the 5-digit zip codes.**
...
**Task** Run the following code to change the datatype of ZCTA5N from numeric to character. What's the datatype of ZCTA5N now? **Answer: now it is a <chr> column.**

...
**Answer: https://www.datawrapper.de/_/jdbWA/** Copy the url of the graphic you published here. It will begin with <https://datawrapper.dwcdn.net/>. Don't copy the browser's current url.
...
md_gov_county <- md_gov_county |>
  clean_names() |>
  mutate(
    total_votes = (cox + moore+ lashar+ wallace+harding+ write_ins), 
...
**Task** Look for "Hide regions without data" under Appearance, and click the slider icon to enable that feature. You should see a map zoomed into Maryland with some counties in various colors. **Answer: This eliminated the other counties and zoomed in on Maryland's results.** Describe the changes from the previous map
...
**Task** Click the "Customize tooltips" button so it expands down. Change {{ fips_code }} to {{ county }} and {{ pctmoore }} to {{ FORMAT(pctmoore, "00.0%")}} **Answer** Describe the changes from the previous map: the title when you hover over a county is the county's name, and the description below is the percentage of Moore voters
...
**Task** Click the "Proceed" button until you get to the "Publish & Embed" tab, then click "Publish Now". Copy the published URL (like you did for the chart above) and paste it below. **Answer: https://www.datawrapper.de/_/EfLrP/** Put the URL of your map here.

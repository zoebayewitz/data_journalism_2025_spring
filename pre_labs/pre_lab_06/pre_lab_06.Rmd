...
**Task** Export the CSV file to your pre_lab_06 folder and read it into R by running the following codeblock. What problems do you see with the data? **Answer: The naming is inconsistent, so a lot of rows don't have corresponding column numbers. Many rows say NA and office/candidate names are inconsistent.**
...
**Task** Get rid of rows we don't need by running the codeblock below. What problems remain? **Answer: The header columns don't have proper titles and one row is called NA.**
...
**Task** Run the codeblock below. What changed, and what issues or problems remain? **Answer: Now, the values are split by party, office, and column name, but some columns like X29 have NA and some of the party values don't make sense.**

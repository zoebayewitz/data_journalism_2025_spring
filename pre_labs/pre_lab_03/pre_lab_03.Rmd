...
**Task** Check for problems that occurred when loading the Texas precinct results by running the following code. How many problems were there, as shown by the number of rows in the output table showing errors? What do you think the problem is that R is describing? Answer below. **Answer: there's 1,640 problems. I think the problem is the number of errors found from the guess_max fucntion **
...
**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **Answer: The errors shown asks for the columns to be specified.**
...
**Task** Run the following codeblock to glimpse the data. What data type is the "district" field? What data type is the "precinct" field? What data type is the "mail" column? Answer below. **Answer: "District" is the district number that people live in. I think the "mail" column is only counted when somebody mailed in their ballot.**
...
**Task** Run the following codeblock to group by mail votes, count the number of precinct results, then sort from highest to lowest on count. How many results are there where mail is NA? What's the implication there? Answer below. **Answer: There are 402,345 results where mail is NA. This probably means that the most people voted without using a mail in ballot.**:
...
**Task** Run the following codeblock to find the number of registered voters grouped by voter status reason, using summarise() to count. Are any of these worthy of exploration to you? Why or why not? Answer below. **Answer: I'm curious about what "legacy data" means, but mostly I'd want to know if felony convicted voters with sentences completed voted immediately after their release, or if they didn't vote until years after their release. **
...
**Task** Run the following codeblock to glimpse the data. What data type is the "amount" field? Answer below. **Answer: it is <chr> and represents the amount they donated to a political campaign**
...
**Answer: inconsistency (capital versus lowercase letters), duplicates, outliers (very small donation amounts)**
...
**Answer: This function changed the name to "x1_linenumber"**
...
new_dataframe <- existing_dataframe |>
  clean_names()
...
**Task** Run the following codeblock to attempt to add up the amount of all contributions. What does the error say when you run this code? What do you think the error means? Answer below. **Answer: the error says that summarise has an error in total_amount=sum(amount); I think this error is because you haven't used group_bby before summarizing**
...
**Task** Run the following codeblock to attempt to change the datatype for the amount field to a number. What is the new data type (three letter code) for amount? Answer below. **Answer: it now includes cents**
...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many are there? What is the donor name? Answer below. **Answer: there are two; the name is Derrick Hamilton**
...
**Task** Run the following codeblock to use distinct() to get rid of duplicate rows. How many rows does the new dataframe have? Answer below. Add a description of what this code does to your reference notebook. **Answer: there's 13 rows. this code condenses the data.**
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, standardizing the field. Look at the difference in the result - what changed? **Answer: the second part of a zip code (last 4 digits) are eliminated so all of the zip codes are the same**
...
**Task** Run the following codeblock to use case_when() to fix misspellings of Conowingo in the "city" field. How many mispellings of Conowingo remain after running this code? Answer below. **Answer: There is one spelling error now**
...
**Task** Run the following codeblock to use case_when() to fix misspellings of Conowingo in the "city" field using both the exact match method and the str_detect() method. How many mispellings of Conowingo remain after running this code? Answer below. Add a description of what this code does to your reference notebook. **Answer: There are no more spelling errors. This makes the data consistent and readable**

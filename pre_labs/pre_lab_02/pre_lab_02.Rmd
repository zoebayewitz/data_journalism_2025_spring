...
glimpse(primary_24)
```
This code organizes the data in 4 different ways, making it readable and used for different purposes.
### Task 4: Mutate
...
**Answer: this code is finding the decimal percentage of votes per candidate, which can be used to determine election outcomes.**
...
```
This code organizes candidates by their percentage of votes on election day in descending order.
How did the results change from the previous task, and why? 

**Answer: the code organized the candidates by descending order of voting percentage, which allows us to see which candidates got the highest percentages of votes.**
...
**Answer: This information is definitely useful in seeing where voter turnout might be lower and which candidates are less favorable. I'm personally surprised Jamie Raskin had such low votes in PG County and would be curious to know why.**
...
**Answer: str_to_lower converts a string to all lowercase; str_to_title capitalizes the first letter of each word; str_to_sentence capitalizes the first letter of each sentence**
...
**Answer: I think the mutate statement first organizes the data by "amount", sorting it by the $ amounts in the code. Then, mutate groups these amounts into "amount_category", labeling it using the yellow values**
...
**Answer: It's important to focus on the final results, as it can be confusing to look at data with all of the categories of voting used**
...
**Answer: the expenditure_date column switched from dating by /slashes/ to -hyphens-**
...
**Answer: the data is showing the expenditures per month, organized by most to least.**

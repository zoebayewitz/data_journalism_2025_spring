...
```{r}
maryland_ppp %>% filter(project_county_name == "Prince Georges")
...
library(readr)
ppp_maryland <- read_csv("data/ppp_maryland.csv", 
    col_types = cols(slug = col_skip(), naics_code = col_character()))
```
...
```{r}
maryland_ppp %>%
  mutate(Month = ceiling_date(date_approved, "month")) %>%
  group_by(Month) %>%
  summarise(total = n()) %>%
  arrange(desc(total))
```
...
```{r}
na_counties <- maryland_ppp %>% filter(is.na(project_county_name))

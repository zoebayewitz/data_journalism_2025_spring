...
**Task** Run the following code to group and count records in the recipient_name column, and filter to examine only recipients that start with W. Name a problem that you see that is preventing proper grouping. **Answer: singular businesses are entered in different ways so they don't group together**
...
**Task** Run the following code to group by recipient_clean and recipient_name, then filter for cities that start with W. Name three changes the refinr function made in the first 20 results. **Answer: capitalized same companies, fixed spacing errors, corrected word order**

```{r}
cleaned_dc_food |>
  group_by(recipient_name, recipient_clean, city) |>
  summarise(
    count=n()
  ) |>
  filter(str_detect(city, "^w|^W")) |>
  arrange(recipient_clean)
```

### Task 7: Use a different refinr method to clean the city_clean field.

**Task** Run the following code to use the n_gram_merge() function to clean the data, then group by city_clean and city to examine cities that start with A. Look at the differences. Does the changes it made vary substantially from the previous use of key_collision_merge? **Answer: yes, this just standardized the capitalization of every city but didn't fix any other errors**
...
cleaned_dc_food |>
  group_by(city, city_clean) |>
  summarise(
    count=n()) |>
  filter(str_detect(city_clean, "^a|^A")) |>
  arrange(city_clean)
```

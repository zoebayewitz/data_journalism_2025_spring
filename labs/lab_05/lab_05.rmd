...
harris <- bind_rows(list(july_harris, august_harris))
...
A1. These columns are the best because they're the most unique identifiable names with the least likelihood for same values. I highly doubt there are any rows with duplicates, but there is a possibility.
...
```{r}
combined |>
  group_by(CountyRegistrationDate, StateRegistrationDate, last_name, first_name, amount) |>
  summarise(count=n()) |>
  filter (CountyRegistrationDate >as.Date("2024-07-20")) |>
  arrange(desc(count))
```

Q3. What are the party and gender breakdowns of those donors? How many donations has Harris gotten from non-Democrats?

A3. 173 females, 141 men; 32 donations from non-Democrats
...
A4: How many voters that registered this year are from Silver Spring? 39 voters.
```{r}
combined |>
  group_by (city) |>
  summarise(count=n())
